...
参照リストに含まれないカラムのみを元データから抽出します。
抽出後のデータは `pattern2_` というプレフィックス付きの Excel ファイルとして保存されます。
...
def keep_mismatched_columns(src_df: pd.DataFrame, ref_norm: set[str]) -> pd.DataFrame:
    """Return a DataFrame containing only columns whose normalized names are not in `ref_norm`.

    Args:
        src_df: Source DataFrame.
        ref_norm: Set of normalized column names to exclude.

    Returns:
        A new DataFrame with only mismatched columns.
    """
...
def main() -> None:
    ...
    result_df = keep_mismatched_columns(src_df, ref_norm)
    out_name = f"pattern2_{src_file.stem}_参照に無い列だけ.xlsx"
